function getHowManyBuildingsWithAViewToTheWest(numbersArray) {
  ...
  if (maxIndex > 0) {
    ...
    let index2 = index1 - 1;
    while (index1 > 0) {
      while (index1 > 0 && index2 >= 0 && numbersArray[index1] >= numbersArray[index2]) {
        index2--;
      }
      if (index2 >= 0) {
        total++;
...
console.log(`le résultat est : ${getHowManyBuildingsWithAViewToTheWest([10, 9, 8, 3, 3, 7, 6, 5, 4, 3, 2, 1])}`);
